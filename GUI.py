                ...
                if selected_product.get_quantity() >= quantity > 0:
                    ...
                    for item in self.cart:
                        if item[0] == selected_product:
                            ...
                            if new_quantity <= selected_product.get_quantity():
                                item[1] = new_quantity
                                messagebox.showinfo("Success",
                                                    f"{quantity} {selected_product.get_name()} added to cart.")
                                self.window.destroy()
                            else:
                                messagebox.showwarning("Error", "Selected quantity exceeds available stock.")
                    ...
                    messagebox.showwarning("Error", "Selected quantity not available.")
            else:
...
class App:
    ...
    def populate_inventory(self):
        # Populate the inventory list with data from savedData.py
...
class RemoveFromCartWindow:
    def __init__(self, parent, cart, callback):
        ...
        # Add label and dropdown menu for selecting product to remove
        tk.Label(self.window, text="Select Product to Remove:").pack(pady=5)
        self.product_options = [item[0].get_name() for item in self.cart]
        self.selected_product = tk.StringVar(value=self.product_options[0])
        self.product_dropdown = ttk.OptionMenu(self.window, self.selected_product, *self.product_options)
    ...
    def confirm_removal(self):
        ...
        for item in self.cart:
            if item[0].get_name() == selected_product_name:
